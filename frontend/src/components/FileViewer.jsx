...
const FileViewer = ({ lesson = {}, attachments = [], isEdit = false, onDelete }) => {
  const files = lesson.files || attachments || [];
  ...
  return (
    <ul className="space-y-2 text-sm max-h-48 overflow-y-auto border p-2 rounded">
      {files.map((file, index) => {
        const filename = file.filename || file.fileName; // support both formats
        const displayName = file.originalname || file.originalName || "Untitled";
        const isLessonFile = !!lesson.files;

        const filePath = isLessonFile
          ? `${baseURL}/uploads/lesson-files/${filename}`
          : `${baseURL}/${(file.filePath || "").replace(/\\/g, "/")}`;
