...
//   const handleFileChange = (e) => {
//     const selected = Array.from(e.target.files);
//     // count only existing persisted attachments (not the ones user already removed in this session)
//     const persistedCount = attachments.filter(a => !(a instanceof File)).length - removedAttachments.length;
//     const totalFiles = selected.length + Math.max(persistedCount, 0);
...
//     const file = attachments[index];
//     if (!(file instanceof File)) {
//       // store id so backend can exclude this from kept list
//       setRemovedAttachments(prev => [...prev, file._id]);
//     }
//     setAttachments(prev => prev.filter((_, i) => i !== index));
//   };
...
const DocumentEdit = () => {
  ...
  return (
    <div className="h-screen w-full flex bg-white text-black" onMouseMove={resize} onMouseUp={stopResize}>
      ...
      {/* Divider */}
      <div className="w-1 bg-gray-300 cursor-col-resize" onMouseDown={() => (resizing.current = true)} onMouseUp={() => { resizing.current = false; localStorage.setItem("leftPanelWidth", leftPanelWidth.toFixed(2)); }} />
